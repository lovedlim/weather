...
function summarizeDay(list) {
  ...
  for (const it of list) {
    ...
    if (it.category === 'SKY') maxSky = Math.max(maxSky, +v || 1);
    if (it.category === 'PTY') maxPty = Math.max(maxPty, +v || 0);
  }
  if (high <= -900) high = null;
  if (low >= 900) low = null;
  return { high, low, maxSky, maxPty };
}
...
app.get('/api/forecast', async (req, res) => {
  ...
  const days = summaries.map((s, i) => {
    ...
    const ci = toConditionIcon({ maxPty: s.maxPty, maxSky: s.maxSky });
